(function() {
    ...
    angular
        .module('app.core')
        .provider('cpNavigationService', cpNavigationServiceProvider)
        .controller('CpNavigationController', CpNavigationController)
        // Vertical
        .directive('cpNavigation', cpNavigationDirective)
        .controller('CpNavigationNodeController', CpNavigationNodeController)
        .directive('cpNavigationNode', cpNavigationNodeDirective)
        .directive('cpNavigationItem', cpNavigationItemDirective)
        //Horizontal
        .directive('cpNavigationHorizontal', cpNavigationHorizontalDirective)
        .controller('CpNavigationHorizontalNodeController', CpNavigationHorizontalNodeController)
        .directive('cpNavigationHorizontalNode', cpNavigationHorizontalNodeDirective)
        .directive('cpNavigationHorizontalItem', cpNavigationHorizontalItemDirective);


    /** @ngInject */
    function cpNavigationServiceProvider() {
        // Inject $log service
    ...
    /** @ngInject */
    function CpNavigationController($scope, cpNavigationService) {
        var vm = this;

        // Data
        if ($scope.root) {
            vm.navigation = cpNavigationService.getNavigation($scope.root);
        } else {
            vm.navigation = cpNavigationService.getNavigation();
        }
        ...
        function toggleHorizontalMobileMenu() {
            angular.element('body').toggleClass('cp-navigation-horizontal-mobile-menu-active');
        }
    }

    /** @ngInject */
    function cpNavigationDirective($rootScope, $timeout, $mdSidenav, cpNavigationService) {
        return {
            restrict: 'E',
            scope: {
                folded: '=',
            ...
            controller: 'CpNavigationController as vm',
            templateUrl: 'app/core/directives/cp-navigation/templates/vertical.html',
            transclude: true,
            compile: function(tElement) {
                tElement.addClass('cp-navigation');

                return function postLink(scope, iElement) {
                    var bodyEl = angular.element('body'),
                        foldExpanderEl = angular.element('<div id="cp-navigation-fold-expander"></div>'),
                        foldCollapserEl = angular.element('<div id="cp-navigation-fold-collapser"></div>'),
                        sidenav = $mdSidenav('navigation');

                    // Store the navigation in the service for public access
                    cpNavigationService.setNavigationScope(scope);
                    ...
                    function init() {
                        ...
                        if (cpNavigationService.getFolded() === null) {
                            cpNavigationService.setFolded(scope.folded);
                        }

                        if (cpNavigationService.getFolded()) {
                            // Collapse everything.
                            // This must be inside a $timeout because by the
                            // time we call this, the 'cpNavigation::collapse'
                            // event listener is not registered yet. $timeout
                            // will ensure that it will be called after it is
                            // registered.
                            $timeout(function() {
                                $rootScope.$broadcast('cpNavigation::collapse');
                            });

                            // Add class to the body
                            bodyEl.addClass('cp-navigation-folded');
                    ...
                    scope.$watch(function() {
                        ...
                        var folded = cpNavigationService.getFolded();
                        ...
                        if (folded) {
                            if (current) {
                                ...
                                var activeItem = cpNavigationService.getActiveItem();
                                if (activeItem) {
                                    activeItem.scope.$emit('cpNavigation::stateMatched');
                                }
                            }
                        }
                    });
                    ...
                    function setFolded(folded) {
                        ...
                        cpNavigationService.setFolded(folded);
                        ...
                        if (folded) {
                            ...
                            // Remove body class
                            bodyEl.removeClass('cp-navigation-folded cp-navigation-folded-open');
                    ...
                    function onFoldExpanderHover(event) {
                        ...
                        // Set folded open status
                        cpNavigationService.setFoldedOpen(true);
                        ...
                        // Add class to the body
                        bodyEl.addClass('cp-navigation-folded-open');
                    ...
                    function onFoldCollapserHover(event) {
                        ...
                        // Set folded open status
                        cpNavigationService.setFoldedOpen(false);

                        // Collapse everything
                        $rootScope.$broadcast('cpNavigation::collapse');

                        // Remove body class
                        bodyEl.removeClass('cp-navigation-folded-open');
    ...
    /** @ngInject */
    function CpNavigationNodeController($scope, $element, $rootScope, $animate, $state, cpNavigationService, $timeout) {
        var vm = this;
        ...
        function init() {
            ...
            if (vm.node.state === $state.current.name || $state.includes(vm.node.state)) {
                ...
                $scope.$emit('cpNavigation::stateMatched');

                // Also store the current active menu item
                cpNavigationService.setActiveItem(vm.node, $scope);
            }

            $scope.$on('cpNavigation::stateMatched', function() {
            ...
            $scope.$on('cpNavigation::collapse', function(event, path) {
            ...
            $scope.$on('$stateChangeSuccess', function() {
                if (vm.node.state === $state.current.name) {
                    ...
                    $rootScope.$broadcast('cpNavigation::collapse', vm.node._path);
                }
        ...
        function collapse() {
            ...
            $scope.$evalAsync(function() {
                ...
                $scope.$broadcast('cpNavigation::collapse');
            });
        }
    ...
    /** @ngInject */
    function cpNavigationNodeDirective() {
        return {
            restrict: 'A',
            bindToController: {
                node: '=cpNavigationNode'
            },
            controller: 'CpNavigationNodeController as vm',
            compile: function(tElement) {
                tElement.addClass('cp-navigation-node');

                return function postLink(scope, iElement, iAttrs, CpNavigationNodeCtrl) {
                    // Add custom classes
                    iElement.addClass(CpNavigationNodeCtrl.getClass());

                    // Add group class if it's a group
                    if (CpNavigationNodeCtrl.group) {
    ...
    /** @ngInject */
    function cpNavigationItemDirective() {
        return {
            restrict: 'A',
            require: '^cpNavigationNode',
            compile: function(tElement) {
                tElement.addClass('cp-navigation-item');

                return function postLink(scope, iElement, iAttrs, CpNavigationNodeCtrl) {
                    // If the item is collapsable...
                    if (CpNavigationNodeCtrl.collapsable) {
                        iElement.on('click', CpNavigationNodeCtrl.toggleCollapsed);
                    }
    ...
    /** @ngInject */
    function cpNavigationHorizontalDirective(cpNavigationService) {
        return {
            restrict: 'E',
            scope: {
                root: '@'
            ...
            controller: 'CpNavigationController as vm',
            templateUrl: 'app/core/directives/cp-navigation/templates/horizontal.html',
            transclude: true,
            compile: function(tElement) {
                tElement.addClass('cp-navigation-horizontal');
    ...
    /** @ngInject */
    function CpNavigationHorizontalNodeController($scope, $element, $rootScope, $state, cpNavigationService) {
        var vm = this;
        ...
        function init() {
            ...
            $scope.$on('cpNavigation::clearActive', function() {
                if (!vm.hasChildren) {
                    return;
                }
            ...
            $scope.$on('$stateChangeSuccess', function() {
                if (vm.node.state === $state.current.name || $state.includes(vm.node.state)) {
                    ...
                    $rootScope.$broadcast('cpNavigation::clearActive');
                }
            });
        }
    ...
    /** @ngInject */
    function cpNavigationHorizontalNodeDirective() {
        return {
            restrict: 'A',
            bindToController: {
                node: '=cpNavigationHorizontalNode'
            },
            controller: 'CpNavigationHorizontalNodeController as vm',
            compile: function(tElement) {
                tElement.addClass('cp-navigation-horizontal-node');

                return function postLink(scope, iElement, iAttrs, CpNavigationHorizontalNodeCtrl) {
                    // Add custom classes
                    iElement.addClass(CpNavigationHorizontalNodeCtrl.getClass());

                    // Add group class if it's a group
                    if (CpNavigationHorizontalNodeCtrl.group) {
                        iElement.addClass('group');
                    }
                };
            }
    ...
    /** @ngInject */
    function cpNavigationHorizontalItemDirective($mdMedia) {
        return {
            restrict: 'A',
            require: '^cpNavigationHorizontalNode',
            compile: function(tElement) {
                tElement.addClass('cp-navigation-horizontal-item');

                return function postLink(scope, iElement, iAttrs, CpNavigationHorizontalNodeCtrl) {
                    iElement.on('click', onClick);

                    function onClick() {
                        if (!CpNavigationHorizontalNodeCtrl.hasChildren || $mdMedia('gt-md')) {
                            return;
                        }
